...
from run_prior_experiment import ROOT, WORK, ARTIFACT, PYTHON, check_sources, run, sha, write_json
...
def main():
    ...
    candidate = next(r for r in frozen["runs"] if r["group"] == "anchored" and r["training_seed"] == 42)
    videos = []
    with (WORK / "gpu.lock").open("w") as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        for seconds in (16, 64):
            for record, name in ((frozen["reference"], "frozen_v5_stones10"),
                                 (candidate, "anchored_seed42_stones10")):
                checkpoint = ROOT / record["checkpoint"]
                if sha(checkpoint) != record["sha256"]:
                    raise ValueError("frozen checkpoint changed")
                name = f"{name}_{seconds}s"
                output = ARTIFACT / "videos" / f"{name}.mp4"
                if output.exists():
                    raise FileExistsError(output)
                command = [str(PYTHON), "scripts/prior_v10.py", "demo", "--task", "Week03-Ant-Prior-Lanes-Demo-v10",
                           "--headless", "--device", args.device, "--seed", "40", "--family", "stepping_stones",
                           "--level", "4", "--cycle-seconds", str(seconds), "--checkpoint", str(checkpoint), "--record", str(output),
                           "env.scene.terrain.terrain_generator.seed=66", f"env.episode_length_s={seconds}.0"]
                ...
                if (int(stream["nb_frames"]), stream["r_frame_rate"], stream["width"], stream["height"]) != (seconds * 30, "30/1", 1280, 720):
                ...
                videos.append({"file": str(output.relative_to(ROOT)), "sha256": sha(output), "checkpoint": record,
                               "seconds": seconds, "command": command, "metadata": metadata, "full_decode_passed": True})
    write_json(ARTIFACT / "video_manifest.json", {
        "geometry": 66, "reset": 40, "family": "stepping_stones", "difficulty": 1.0,
        "selection": "predeclared seed42; uncut16s and64s including any failures/resets; no selection by results",
