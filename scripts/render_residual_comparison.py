...
from run_residual_experiment import ROOT, WORK, ARTIFACT, PYTHON, check_sources, run, sha, write_json
...
def main():
    ...
    candidate = next(r for r in frozen["runs"] if r["group"] == "residual_footmap" and r["training_seed"] == 42)
    videos = []
    with (WORK / "gpu.lock").open("w") as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        for record, name in ((frozen["reference"], "frozen_v5_stones10"),
                             (candidate, "residual_footmap_seed42_stones10")):
            ...
            command = [str(PYTHON), "scripts/residual_v8.py", "demo", "--task", "Week03-Ant-Residual-Lanes-Demo-v8",
                       "--headless", "--device", args.device, "--seed", "36", "--family", "stepping_stones",
                       "--level", "4", "--cycle-seconds", "16", "--checkpoint", str(checkpoint), "--record", str(output),
                       "env.scene.terrain.terrain_generator.seed=62"]
    ...
    write_json(ARTIFACT / "video_manifest.json", {
        "geometry": 62, "reset": 36, "family": "stepping_stones", "difficulty": 1.0,
