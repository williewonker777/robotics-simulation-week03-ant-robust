# v10 independent primary and horizon evidence review
...
- Result SHA-256: `d384de294dbf407b4847a6d5cd1eea5b4ee9d94035c4f9a6a20c8a9917f2879f`
- Result status: `pass`, mismatches: `0`
- Final command ledger: 49/49 return code zero (13 preparation/training,
  14 primary evaluation, 14 horizon evaluation, 4 video render, 4 full decode)
- Primary evidence: 14 distinct JSON files / 2,450 first episodes
- Horizon evidence: 14 distinct JSON files / 140 first episodes
- Preserved primary-only result: `independent_audit_primary.json`, SHA-256
  `2288464e1a3af5cd8d16c3825327b48d4bd8c7ffa9a67a2c35cbf1f73a4fa1b7`
- Scope: primary 16-second benchmark plus the separately predeclared 64-second
  stepping-stones1.0 diagnostic. Horizon evidence is explicitly non-gating.
...
## Separate 64-second stepping-stones1.0 diagnostic

The optional `--horizon` audit passed with zero mismatches over 14 distinct raw
files / 140 first episodes. It rechecked the 64-second/3,840-step condition,
10-environment assignment, geometry/reset pairs, checkpoint/mode identities,
strict boolean arrays, finite returns, `episode_steps == episode_lengths`,
auto-reset final evidence, 16-second censoring, first-hit time bounds and
one-before-six hit ordering. All 14 horizon commands began after model freeze and
returned zero. The independently recomputed totals exactly match
`horizon_summary.json`.

| Policy | Strict one | Strict six | Falls | Lane exits | Full 64 s survival |
|---|---:|---:|---:|---:|---:|
| frozen v5 | 13/20 (65.0%) | 0/20 (0.0%) | 5/20 (25.0%) | 0/20 (0.0%) | 15/20 (75.0%) |
| free | 19/60 (31.7%) | 14/60 (23.3%) | 29/60 (48.3%) | 12/60 (20.0%) | 31/60 (51.7%) |
| anchored | 23/60 (38.3%) | 15/60 (25.0%) | 31/60 (51.7%) | 5/60 (8.3%) | 29/60 (48.3%) |

Distance-only first-hit counts exceed strict final successes because an episode
may cross a threshold and later fall or leave its lane. Frozen v5 reached one
tile in 15/20 episodes but strictly finished beyond it in 13/20; it never reached
six tiles. Free reached one/six in 46/23 episodes but strictly finished them in
19/14. Anchored reached one/six in 48/21 but strictly finished them in 23/15.
Thus first-hit evidence is not being presented as safe completion.

Conditional on reaching the threshold, mean first-hit times were:

| Policy | One-tile hits/time | Six-tile hits/time |
|---|---:|---:|
| frozen v5 | 15, 13.03 s | 0, n/a |
| free | 46, 9.99 s | 23, 51.67 s |
| anchored | 48, 10.80 s | 21, 56.72 s |

At the 16-second snapshot, only episodes still alive are observed: v5 17/20 with
mean 14.19 m, free 47/60 with mean 18.33 m, and anchored 49/60 with mean 16.59 m.
These are censored conditional means, not unconditional policy means.

The paired horizon result is heterogeneous:

| Seed | Policy | Strict one | Strict six | Falls | Lane | Survival |
|---|---|---:|---:|---:|---:|---:|
| 42 | free | 5/20 | 4/20 | 13/20 | 3/20 | 7/20 |
| 42 | anchored | 7/20 | 6/20 | 10/20 | 1/20 | 10/20 |
| 43 | free | 4/20 | 3/20 | 10/20 | 6/20 | 10/20 |
| 43 | anchored | 8/20 | 5/20 | 10/20 | 1/20 | 10/20 |
| 44 | free | 10/20 | 7/20 | 6/20 | 3/20 | 14/20 |
| 44 | anchored | 8/20 | 4/20 | 11/20 | 3/20 | 9/20 |

Anchoring sharply reduced long-horizon lane exits relative to free (5 versus 12)
and slightly increased strict one-/six-tile totals (23/15 versus 19/14), but it
did **not** sustain the primary benchmark's fall reduction: anchored had 31 falls
versus free's 29 and lower full-horizon survival (29 versus 31). Seed 44 was
clearly worse under anchoring. Frozen v5 remained safest for falls, lane
containment, and survival but produced no strict six-tile completions. These
tradeoffs support neither promotion nor a claim of long-horizon safety
improvement.

The horizon sample reuses only two terrain maps and uses a different 10-environment
reset assignment from the primary 175-environment benchmark. It is a diagnostic,
not additional training, an independent-map expansion, a relaxed primary metric,
or a statistical significance test.


## Predeclared qualitative presentation

The four predeclared, uncut geometry66/reset40/stepping-stones1.0 videos exist and
passed full decode. They compare the fixed seed42 anchored policy with frozen v5;
no video, seed, or failure interval was selected after seeing results.

| File | Frames / format | SHA-256 |
|---|---|---|
| `frozen_v5_stones10_16s.mp4` | 480, 30 fps, 1280x720 | `01152b09c0384b415837ed25eee0bf36c36e97c9a5b0d3b59573c26e68e9c93a` |
| `anchored_seed42_stones10_16s.mp4` | 480, 30 fps, 1280x720 | `a73394f39438f8e8951794bd72651b516d635ab8997c62f8176b463abb5bd878` |
| `frozen_v5_stones10_64s.mp4` | 1,920, 30 fps, 1280x720 | `8b04637a874eb2bb267ada29733b5f474b2fe2d6a7200c72f3915d8d6b72567a` |
| `anchored_seed42_stones10_64s.mp4` | 1,920, 30 fps, 1280x720 | `2689cf4aaf76532360c8964924d877e158cc048a11c97651994238f8935f070e` |

The final displayed values were 8.3 m/reset2 versus 17.6 m/reset1 at 15.9 s,
and 43.6 m/reset2 versus 68.1 m/reset1 at 63.9 s (v5 versus anchored). These
values describe the **current episode after the displayed resets**. They are not
the first-episode distance, strict one-/six-tile benchmark success, or additional
statistical evidence. The raw first-episode JSON and independently audited tables
above remain authoritative.

The local viewer was generated at `artifacts/terrain_demo/prior_v10/index.html`.
Its videos are qualitative one-environment rollouts and do not alter either gate.

## Recommendation

Retain the frozen v5 policy as the recommended/default policy. Preserve all v10
models and raw evidence as an experimental result. The audited 64-second
diagnostic explains that learned policies can eventually traverse some hard
stones, but it does not repair the failed primary promotion gate and exposes that
the anchored fall advantage is not sustained over the longer horizon.
